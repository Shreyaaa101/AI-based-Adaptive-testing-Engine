...
class OpenRouterAPI:
    ...
    def generate_questions(self, text_content: str) -> Tuple[List[Dict], str]:
        ...
        """
        try:
            # Use the whole extracted text (no truncation)
            limited_text = text_content

            prompt = f"""Based on the following text, generate exactly 20 multiple-choice questions in JSON format. 
Each question should test understanding of the material at different difficulty levels (0.1 to 0.9).
...
Make sure to vary the difficulty levels from 0.1 (easiest) to 0.9 (hardest). 
